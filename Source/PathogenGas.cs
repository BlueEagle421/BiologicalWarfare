...
namespace USH_BW
{
    public class PathogenGas : Gas
    {
        ...
        private PathogenGasExtension _cachedExtension;
        public PathogenGasExtension PathogenGasExt
        {
            get
            {
                _cachedExtension ??= def.GetModExtension<PathogenGasExtension>();
        ...
        private void InfectionTick(int delta)
        {
            if (Destroyed)
                return;

            _infectionTick += delta;

            if (_infectionTick < PathogenGasExt.PeriodTicks)
                return;
    ...
    public class PathogenGasExtension : DefModExtension
    {
        public HediffDef HediffDef;
        public float SeverityPerPeriod;
        public int PeriodTicks = 120;
    }
}